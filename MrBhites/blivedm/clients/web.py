...
class BLiveClient(ws_base.WebSocketClientBase):
    ...
    # Mrbhite
    # time stap, used to calculate w_rid
    wts = round(time.time())
    ...
    # Mrbhite
    # 2025/6/9
    # describe: code below is used to get WbiKeys for the request to DANMAKU_SERVER_CONF_URL
    ...
    async def get_w_rid(self):
        '为请求参数进行 wbi 签名'
        img_key, sub_key = await self.getWbiKeys()
        key = img_key + sub_key
        mixin_key = self.getMixinKey(key)
        params={}
        params['id'] = self._room_id
        params['type'] = 0
        params['web_location'] = 444.8
        params['wts'] = self.wts                                    # 添加 wts 字段
